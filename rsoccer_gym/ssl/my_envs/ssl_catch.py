...
class SSLCatchEnv(SSLBaseEnv):
    ...
    def _calculate_reward_and_done(self):
        ...
        if abs(active_player.y) > half_wid or abs(active_player.x) > half_len:
        ...
        elif self.possession_robot_idx == self.active_robot_idx:
            done = True
            self.reward_shaping_total['goal'] += 1
            reward = 50

        elif self.last_frame is not None:
            move_to_ball_rw = self.__move_to_ball_rw()
            ...
            reward = move_to_ball_rw + energy_rw
    ...
    def _get_initial_positions_frame(self):
        ...
        pos_frame.ball = Ball(x=x(), y=y(), v_x=random.uniform(-2, 2), v_y=random.uniform(-2, 2))
        ...
        for i in range(self.n_robots_blue):
            ...
            while places.get_nearest(pos)[1] < min_dist:
                pos = (x(), y())
            places.insert(pos)
            robot_list.append(Robot(x=pos[0], y=pos[1], theta=theta()))

        def dis_to_ball(robot, ball_x=pos_frame.ball.x, ball_y= pos_frame.ball.y):
            robot_pos = [robot.x, robot.y]
            ball_pos = [ball_x,ball_y]
            return np.sqrt(sum((robot_pos - ball_pos) ** 2 for robot_pos, ball_pos in zip(robot_pos, ball_pos)))

        robot_list.sort(key=lambda x: dis_to_ball(x))

        for i in range(self.n_robots_blue):
            pos_frame.robots_blue[i] = robot_list[i]
