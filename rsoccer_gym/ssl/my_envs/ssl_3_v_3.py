...
class SSL3v3Env(SSLBaseEnv):
    ...
    def __init__(self, field_type=2):
        ...
        self.opponents = [ramdom_agent,ramdom_agent,ramdom_agent]
    ...
    def construct_observation(self,main_robot_team,main_robot_idx):
        ...
        # 自己的观察
        observation.append(self.norm_pos(main_robot.x))
        observation.append(self.norm_pos(main_robot.y))
        observation.append(
            np.sin(np.deg2rad(main_robot.theta))
        )
        observation.append(
            np.cos(np.deg2rad(main_robot.theta))
        )
        observation.append(self.norm_v(main_robot.v_x))
        observation.append(self.norm_v(main_robot.v_y))
        observation.append(self.norm_w(main_robot.v_theta))

        for robot in teammates:
            observation.append(self.norm_pos(robot.x))
            ...
            observation.append(self.norm_v(robot.v_x))
            observation.append(self.norm_v(robot.v_y))
            observation.append(self.norm_w(robot.v_theta))
    ...
    def _frame_to_observations(self):
        ...
        observation = self.construct_observation("blue",self.active_robot_idx)
